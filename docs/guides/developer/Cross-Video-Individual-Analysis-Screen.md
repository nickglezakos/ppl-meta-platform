# Cross-Video Individual Analysis Screen - Technical Documentation
...
1. **Individuals Tab** (Index 0)
   ...
   - **Selection-based Actions** (NEW in v2.19.46):
     - Checkbox selection for 1+ individuals
     - Floating Action Button appears on selection
     - Actions dialog with two operations:
       - **Add to Group**: Available when 1+ individuals selected
       - **Merge Individuals**: Available when 2+ individuals selected
...
## Actions Dialog (v2.19.46)

### Overview

The Actions Dialog provides a centralized interface for performing operations on selected individuals in the Individuals Tab. This feature was added in version 2.19.46 to support both group management and individual merging workflows.
...
**Key Points:**
- Button appears when **1 or more** individuals are selected
- Shows count of selected individuals in label
- Icon: `Icons.admin_panel_settings`
- Color: Blue

### Actions Dialog Structure
...
showDialog(
  ...
  builder: (context) => AlertDialog(
    ...
    content: Column(
      ...
      children: [
        // Add to Group button (always available)
        ElevatedButton.icon(
          onPressed: () {
        ...
        // Merge Individuals button (conditional)
        if (canMerge)  // Only if 2+ selected
          ElevatedButton.icon(
            onPressed: () {
              Navigator.of(context).pop();
...
**Availability:** Always available when dialog opens (1+ selected)
...
**Functionality:**
- Opens `_showAddToGroupDialog()` method
- Currently displays placeholder UI
- Future implementation will allow:
  - Creating named groups of individuals
  - Adding/removing individuals from existing groups
  - Searching and filtering by group
  - Generating group-level analytics
...
showDialog(
  ...
  builder: (context) => AlertDialog(
    ...
    content: Column(
      ...
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text('Add ${_selectedIndividuals.length} individual(s) to a group'),
...
**Availability:** Only available when **2 or more** individuals selected

**Visual Design:**
- Button color: Blue
- Icon: `Icons.merge`
- Label: "Merge X Individuals" (dynamic count)

**Functionality:**
- Opens `_showMergeConfirmationDialog()` method
- Existing merge workflow (unchanged from previous versions)
- Consolidates selected individuals into single MVR person
- Uses similarity threshold for validation
...
if (canMerge)
  ElevatedButton.icon(/* merge button */),
...
#### Single Individual Selection

1. User taps checkbox on one individual card
2. FloatingActionButton appears: "Actions (1)"
3. User taps Actions button
4. Dialog shows:
   - **Add to Group** button (green, enabled)
   - Help text: "Select 2 or more individuals to merge"
5. User can only add to group

#### Multiple Individual Selection

1. User taps checkboxes on 2+ individual cards
2. FloatingActionButton appears: "Actions (X)" (X = selection count)
3. User taps Actions button
4. Dialog shows:
   - **Add to Group** button (green, enabled)
   - **Merge X Individuals** button (blue, enabled)
5. User can choose either operation

### State Management

**Selection State:**
```dart
// Track selected individuals for merging/grouping
...
void _toggleIndividualSelection(String individualUuid) {
  setState(() {
    if (_selectedIndividuals.contains(individualUuid)) {
      _selectedIndividuals.remove(individualUuid);
    } else {
      _selectedIndividuals.add(individualUuid);
    }
  });
}
...
**Key Methods:**
- `_showActionsDialog()` (lines ~1718-1785)
- `_showAddToGroupDialog()` (lines ~1787-1833)
- `_showMergeConfirmationDialog()` (existing merge logic)

**Related Code:**
```dart
// FloatingActionButton condition (line ~119)
floatingActionButton: _isCrossVideoMode && _selectedIndividuals.isNotEmpty

// Actions dialog method (lines 1718-1785)
Future<void> _showActionsDialog() async {
  final bool canMerge = _selectedIndividuals.length >= 2;
  // ... dialog implementation
}

// Add to group placeholder (lines 1787-1833)
...
1. **Single Selection Test:**
   ```
   ✓ Select 1 individual
   ✓ Verify Actions button appears with "(1)"
   ✓ Tap Actions
   ✓ Verify only "Add to Group" is enabled
   ✓ Verify merge help text is shown
   ```

2. **Multiple Selection Test:**
   ```
   ✓ Select 2+ individuals
   ✓ Verify Actions button shows correct count
   ✓ Tap Actions
   ✓ Verify both buttons are enabled
   ✓ Verify merge button shows count
   ```

3. **Add to Group Test:**
   ```
   ✓ Select individuals
   ✓ Tap Actions → Add to Group
...
4. **Merge Test:**
   ...
   ✓ Select 2+ individuals
   ✓ Tap Actions → Merge
   ✓ Verify existing merge dialog appears
...
1. Create new groups with custom names
2. Add individuals to existing groups
3. Remove individuals from groups
4. Group search and filtering
5. Group-level analytics dashboard
6. Bulk group operations

**Backend Requirements:**
- New `groups` table in VMeta database
- Many-to-many relationship: `group_members` table
- Group CRUD API endpoints
- Group search/filter endpoints
...
- Export selected individuals (JSON/CSV)
- Delete selected individuals
- Tag/label individuals
- Compare selected individuals (side-by-side view)
- Generate report for selected individuals
...
- **Latest Changes:** Actions dialog with "Add to Group" and conditional "Merge" buttons
