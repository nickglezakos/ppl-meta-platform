...
```bash
gh workflow run platform-release.yml -f platform_version=2.25.80
gh run watch
```
...
RELEASE_TAG=2.25.80 ./scripts/build_windows_installer_images.sh

# Push
RELEASE_TAG=2.25.80 ./scripts/push_protected_service_images.sh
```

### Verify manifests

```bash
./scripts/verify_platform_release.sh 2.25.80
```
...
for s in node media gateway orchestrator discovery communications frontend \
  ...
  docker manifest inspect "ghcr.io/nickglezakos/ppl-meta-platform/ppl-meta-$s:2.25.80" >/dev/null \
    && echo "OK ppl-meta-$s:2.25.80" \
    || echo "MISSING ppl-meta-$s:2.25.80"
done
```
...
7. Confirm generated `.env.windows` has `RELEASE_TAG=2.25.80` and `DISCOVERY_PORT=8006`.
...
- [ ] Dispatch `platform-release.yml` with `platform_version=2.25.80` (or local build/push)
- [ ] All twelve manifests exist for `2.25.80` (`./scripts/verify_platform_release.sh 2.25.80`)
