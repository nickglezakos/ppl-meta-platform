...
async def video_stream_session(
    ...
    """Stream video from camera using session-based authentication."""
    logger.info("🎬 ENTRY: video_stream_session called for device_id=%s", device_id)
    
    # Validate session
    session = session_manager.validate_session(session_id, device_id)
    if not session:
        logger.error("❌ Session validation failed for device_id=%s", device_id)
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid or expired streaming session",
        )
    
    logger.info("✅ Session validation passed for device_id=%s", device_id)
    ...
    async def generate_frames():
        """Generate video frames for streaming with camera resource management."""
        logger.info("🎥 Starting frame generation with resource fix for %s", device_id)
        
        try:
            # CAMERA RESOURCE FIX: Create fresh connection for streaming
            # Release any existing connection to avoid resource locks
            logger.info("🔧 Ensuring fresh camera connection for streaming...")
            
            # Step 1: Disconnect any existing connection
            await camera_service.disconnect_camera(device_id)
            await asyncio.sleep(0.2)  # Allow camera to release
            
            # Step 2: Create fresh connection with macOS-optimized settings
            try:
                # Get camera index from detected cameras
                camera_info = camera_service.detected_cameras.get(device_id)
                if not camera_info:
                    logger.error("Camera info not found for %s", device_id)
                    return
                
                index = int(camera_info.get("connection_string", "0"))
                logger.info("Creating fresh VideoCapture for index %d", index)
                
                # Create VideoCapture with macOS-optimized backend
                cap = cv2.VideoCapture(index, cv2.CAP_AVFOUNDATION)
                
                if not cap.isOpened():
                    logger.info("AVFoundation backend failed, trying default...")
                ...
                camera_service.active_connections[device_id] = cap
                logger.info("✅ Fresh camera connection established")
                
            except Exception as e:
                logger.error("Failed to create camera connection: %s", str(e))
                return
            ...
            logger.info("Starting video stream with quality %s (%dx%d@%dfps)", quality, width, height, fps)
            ...
            while True:
                # Validate session is still active
                if not session_manager.validate_session(session_id, device_id):
                    logger.warning("Session expired during streaming")
                    break
                ...
                try:
                    ...
                    logger.warning("Frame read timeout (attempt %d/%d)", consecutive_failures, max_consecutive_failures)
                ...
                if not ret:
                    consecutive_failures += 1
                    logger.warning("Failed to read frame (attempt %d/%d)", consecutive_failures, max_consecutive_failures)
            ...
            logger.error("Error in video stream generation: %s", str(e))
            yield (
                b"--frame\r\n"
    ...
    try:
        ...
        logger.error("Error setting up video stream: %s", str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Error setting up video stream: {e}",
        )

@router.get("/{device_id}/video-session-original/{session_id}")
async def video_stream_session_original(
    device_id: str,
    session_id: str,
    quality: str = "medium",
    db: Session = Depends(get_db),
):
    """Original stream video function - renamed for testing."""
    logger.info("🎬 ORIGINAL: video_stream_session called for device_id=%s", device_id)
    ...
    logger.info("✅ Step 1 PASSED: Session validation successful")

    # Check if this is a mobile camera
    logger.info("🔍 Step 2: Checking camera type...")
    camera = db.query(Camera).filter(Camera.device_id == device_id).first()
    if camera and camera.camera_type == CameraType.MOBILE:
        logger.info("📱 Mobile camera detected, delegating...")
        return await handle_mobile_camera_stream_session(device_id, session_id, quality)
    
    logger.info("✅ Step 2 PASSED: Camera type check completed")

    # Step 3: Test response before camera operations
    async def step_by_step_test():
        logger.info("🧪 Step 3: Testing response generation...")
        yield b"--frame\r\n"
        yield b"Content-Type: text/plain\r\n\r\n"
        yield b"STEP 3 PASSED - Before camera operations\r\n"
        
        # Step 4: Test camera connection
        logger.info("🧪 Step 4: Testing camera connection...")
        cap = await camera_service.get_camera_stream(device_id)
        if cap:
            logger.info("✅ Step 4 PASSED: Camera connection exists")
            yield b"--frame\r\n"
            yield b"Content-Type: text/plain\r\n\r\n"
            yield b"STEP 4 PASSED - Camera connection exists\r\n"
        else:
            logger.error("❌ Step 4 FAILED: No camera connection")
            yield b"--frame\r\n"
            yield b"Content-Type: text/plain\r\n\r\n"
            yield b"STEP 4 FAILED - No camera connection\r\n"
            return
            
        # Step 5: Test frame reading (with timeout)
        logger.info("🧪 Step 5: Testing frame reading...")
        try:
            ret, frame = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(None, cap.read),
                timeout=2.0
            ...
            if ret:
                logger.info("✅ Step 5 PASSED: Frame read successful")
                yield b"--frame\r\n"
                yield b"Content-Type: text/plain\r\n\r\n"
                yield b"STEP 5 PASSED - Frame read successful\r\n"
            else:
                logger.error("❌ Step 5 FAILED: Frame read returned False")
                yield b"--frame\r\n"
                yield b"Content-Type: text/plain\r\n\r\n"
                yield b"STEP 5 FAILED - Frame read returned False\r\n"
        except asyncio.TimeoutError:
            logger.error("❌ Step 5 FAILED: Frame read timeout")
            yield b"--frame\r\n"
            yield b"Content-Type: text/plain\r\n\r\n"
            yield b"STEP 5 FAILED - Frame read timeout\r\n"
        except Exception as e:
            logger.error("❌ Step 5 FAILED: Frame read exception: %s", str(e))
            yield b"--frame\r\n"
            yield b"Content-Type: text/plain\r\n\r\n"
            yield f"STEP 5 FAILED - Exception: {str(e)}\r\n".encode()
        
        logger.info("🎉 All diagnostic steps completed!")
        yield b"--frame\r\n"
        yield b"Content-Type: text/plain\r\n\r\n"
        yield b"DIAGNOSTIC COMPLETE - All steps tested\r\n"
    ...
    try:
        ...
        logger.error("❌ Error in step-by-step test: %s", str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Diagnostic test failed: {e}",
        )
    ...
    async def generate_frames():
        """Generate video frames for streaming."""
        logger.info("🎥 Starting frame generation for camera %s", device_id)
        
        # First, send a test frame to verify the endpoint is working
        yield (
            b"--frame\r\n"
            b"Content-Type: text/plain\r\n\r\n"
            b"STREAMING TEST - ENDPOINT WORKING\r\n"
        )
        ...
        try:
            ...
            try:
                ...
                if not cap.isOpened():
                    logger.error("Failed to open camera with AVFoundation backend")
                    # Fallback to default backend
                    cap = cv2.VideoCapture(index)
                ...
                # Store the connection
                camera_service.active_connections[device_id] = cap
                logger.info("✅ Fresh camera connection established for %s", device_id)
            ...
            while True:
                ...
                # Add timeout for frame reading to prevent hanging
                try:
                    # Use asyncio.wait_for to add timeout to blocking cap.read()
                    ret, frame = await asyncio.wait_for(
                    ...
                    consecutive_failures += 1
                    logger.warning(
                        "Frame read timeout for camera %s (attempt %d/%d)",
                        device_id, consecutive_failures, max_consecutive_failures
                    )
                    
                    if consecutive_failures >= max_consecutive_failures:
                        logger.error(
                            "Too many consecutive frame read timeouts for camera %s",
                            device_id
                        )
                        break
                ...
                if not ret:
                    consecutive_failures += 1
                    logger.warning(
                        "Failed to read frame from camera %s (attempt %d/%d)",
                        device_id, consecutive_failures, max_consecutive_failures
                    )
                    
                    if consecutive_failures >= max_consecutive_failures:
                        logger.error(
                            "Too many consecutive frame read failures for camera %s",
                            device_id
                        )
                        break
...
@router.get("/{device_id}/camera-diagnostic")
async def camera_diagnostic(
    device_id: str,
    current_user: Dict = Depends(get_current_user)
) -> Dict:
    """Diagnostic endpoint to test camera access without streaming."""
    logger.info("🔧 DIAGNOSTIC: Testing camera access for %s", device_id)
    
    try:
        # Step 1: Check if camera exists in active connections
        cap = await camera_service.get_camera_stream(device_id)
        connection_status = "exists" if cap else "missing"
        logger.info("📊 Active connection status: %s", connection_status)
        
        # Step 2: Test if connection is opened
        is_opened = cap.isOpened() if cap else False
        logger.info("📊 Connection opened status: %s", is_opened)
        
        # Step 3: Try direct camera creation (bypassing service)
        logger.info("📊 Testing direct camera creation...")
        direct_cap = None
        direct_success = False
        try:
            # Get camera info to know the index
            camera_info = camera_service.detected_cameras.get(device_id)
            if camera_info:
                index = int(camera_info.get("connection_string", "0"))
                logger.info("📊 Creating direct VideoCapture with index %d", index)
                direct_cap = cv2.VideoCapture(index)
                direct_success = direct_cap.isOpened()
                logger.info("📊 Direct camera creation success: %s", direct_success)
                if direct_cap:
                    direct_cap.release()  # Clean up immediately
            else:
                logger.warning("📊 No camera info found for %s", device_id)
        except Exception as e:
            logger.error("📊 Direct camera creation failed: %s", str(e))
        
        # Step 4: Check active connections state
        active_connections_count = len(camera_service.active_connections)
        logger.info("📊 Total active connections: %d", active_connections_count)
        
        return {
            "device_id": device_id,
            "diagnostics": {
                "service_connection_exists": connection_status == "exists",
                "service_connection_opened": is_opened,
                "direct_camera_creation": direct_success,
                "total_active_connections": active_connections_count,
                "camera_info_exists": device_id in camera_service.detected_cameras,
            },
            "recommendations": {
                "service_connection_stale": connection_status == "exists" and not is_opened,
                "need_fresh_connection": not direct_success,
                "camera_resource_conflict": connection_status == "exists" and is_opened and not direct_success,
            },
            "status": "diagnostic_complete"
        }
        
    except Exception as e:
        logger.error("❌ Diagnostic failed: %s", str(e))
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Camera diagnostic failed: {e}",
        )
