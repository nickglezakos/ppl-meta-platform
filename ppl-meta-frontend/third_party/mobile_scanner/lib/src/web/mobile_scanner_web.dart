...
class MobileScannerWeb extends MobileScannerPlatform {
  ...
  Future<MediaStream> _prepareVideoStream(CameraFacing cameraDirection) async {
    // `window.navigator.mediaDevices` is typed non-nullable by package:web,
    // but the browser returns `undefined` on insecure origins (anything other
    // than `localhost`/`127.0.0.1` over HTTP, or HTTPS). Reading it directly
    // through the typed getter crashes with a JSObject cast error before any
    // null check can run, so probe the raw JS property first.
    final mediaDevicesRaw = (window.navigator as JSObject).getProperty<JSAny?>(
      'mediaDevices'.toJS,
    );
    if (mediaDevicesRaw == null) {
      final origin = window.location.origin;
      final isSecure = window.isSecureContext;
      throw MobileScannerException(
        errorCode: MobileScannerErrorCode.unsupported,
        errorDetails: MobileScannerErrorDetails(
          message:
              'Camera access is unavailable in this browsing context '
              '(origin: $origin, secureContext: $isSecure). '
              'Open the app over https:// or via http://localhost to allow '
    ...
    final maxTouchPoints = window.navigator.maxTouchPoints;
    final isAppleMobileDevice =
        userAgent.contains('iPad') ||
        userAgent.contains('iPhone') ||
        userAgent.contains('iPod') ||
        (platform == 'MacIntel' && maxTouchPoints > 1);
    final isWebKitBrowser = userAgent.contains('AppleWebKit');
    final shouldUseFacingModeConstraint =
        isAppleMobileDevice && isWebKitBrowser;

    final MediaStreamConstraints constraints;

    if (shouldUseFacingModeConstraint) {
