...
class NetworkSettingsSection extends ConsumerWidget {
              ...
              child: DataTable(
                ...
                columns: [
                  DataColumn(
                    label: Expanded(
                      flex: 3,
                      child: Text('Service', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    ),
                  ),
                  DataColumn(
                    label: Expanded(
                      flex: 2,
                      child: Text('Status', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    ),
                  ),
                  DataColumn(
                    label: Expanded(
                      flex: 4,
                      child: Text('URL', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    ),
                  ),
                  DataColumn(
                    label: Expanded(
                      flex: 2,
                      child: Text('Version', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    ),
                  ),
                  DataColumn(
                    label: Expanded(
                      flex: 2,
                      child: Text('VPN IP', style: TextStyle(fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                    ),
                  ),
                ],
      ...
      child: Center(
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            const Text('Loading services...', style: TextStyle(color: AppColors.textSecondary)),
          ],
        ),
      ),
    );
      ...
      child: Center(
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            const Text('Failed to load services', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: AppColors.error)),
            const SizedBox(height: 8),
            Text(error.toString(), style: const TextStyle(color: AppColors.textSecondary, fontSize: 14), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
      ...
      child: Center(
        child: Column(
          children: [
            ...
            const SizedBox(height: 16),
            const Text('No services discovered', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600, color: AppColors.textSecondary)),
            const SizedBox(height: 8),
            const Text('The discovery service is running but no services were found.', style: TextStyle(color: AppColors.textSecondary, fontSize: 14), textAlign: TextAlign.center),
          ],
        ),
      ),
    );
  ...
  Widget _buildServiceCell(ServiceInfo service) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(service.name, style: const TextStyle(fontWeight: FontWeight.w500, fontSize: 12, color: AppColors.textPrimary), maxLines: 2, overflow: TextOverflow.visible),
          if (service.serviceType.isNotEmpty) ...[
            const SizedBox(height: 2),
            Text(service.serviceType, style: const TextStyle(fontSize: 10, color: AppColors.textSecondary), maxLines: 1, overflow: TextOverflow.ellipsis),
          ],
        ],
      ),
    );
    ...
    return Padding(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(color: isHealthy ? AppColors.success : AppColors.error),
        ),
        child: Text(service.status, textAlign: TextAlign.center, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: isHealthy ? AppColors.success : AppColors.error), overflow: TextOverflow.ellipsis, maxLines: 1),
      ),
    );
  }
  ...
  Widget _buildUrlCell(ServiceInfo service) {
    return Padding(
      ...
      child: Column(
        ...
        mainAxisSize: MainAxisSize.min,
        children: [
          Text(service.baseUrl, style: const TextStyle(fontFamily: 'monospace', fontSize: 11, color: AppColors.primary), maxLines: 2, overflow: TextOverflow.visible),
          const SizedBox(height: 2),
          Text('${service.host}:${service.port}', style: const TextStyle(fontSize: 10, color: AppColors.textSecondary), maxLines: 1, overflow: TextOverflow.ellipsis),
        ],
      ),
    );
  }
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              const Text('VPN Mesh Active', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.success)),
            ],
          ),
          const SizedBox(height: 12),
          ...
          Row(
            children: [
              const Text('Tailscale', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500)),
              const Spacer(),
              Container(width: 8, height: 8, decoration: BoxDecoration(color: status.enrolled ? AppColors.success : Colors.grey, shape: BoxShape.circle)),
              const SizedBox(width: 6),
              Text(status.enrolled ? 'Connected' : 'Disconnected', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w500, color: status.enrolled ? AppColors.success : Colors.grey)),
            ],
          ),
        ],
      ),
  ...
  /// VPN Mesh Peers — online/offline list with rename and delete.
  Widget _buildVpnPeersCard() {
    return Consumer(
      builder: (context, ref, _) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  const Icon(Icons.hub, color: AppColors.secondary, size: 20),
                  const SizedBox(width: 8),
                  const Text('Mesh Peers', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
                  const Spacer(),
                  TextButton.icon(
                    onPressed: () => ref.invalidate(vpnPeersProvider),
                    ...
                    return const Padding(
                      ...
                      child: Row(
                        children: [
                          ...
                          SizedBox(width: 8),
                          Text('No peers found', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                        ],
                      ),
                    );
                  }
                  return Column(
                    children: peers.map((peer) => _buildPeerRow(context, ref, peer)).toList(),
                  );
                },
                loading: () => const Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      SizedBox(width: 12),
                      Text('Loading peers...', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                    ],
                  ),
                ),
  ...
  Widget _buildPeerRow(BuildContext context, WidgetRef ref, VpnPeerInfo peer) {
    return Container(
      ...
      child: Row(
        children: [
          Icon(
            peer.online ? Icons.check_circle : Icons.cancel,
            color: peer.online ? AppColors.success : AppColors.error,
            size: 20,
          ),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  '${peer.hostname}.eyenet-vpn.local',
                ...
                Row(
                  children: [
                    ...
                    Container(
                      ...
                      decoration: BoxDecoration(
                        color: peer.online ? AppColors.success.withValues(alpha: 0.15) : AppColors.error.withValues(alpha: 0.15),
                        borderRadius: BorderRadius.circular(4),
                        border: Border.all(color: peer.online ? AppColors.success : AppColors.error, width: 0.5),
                      ),
                      child: Text(
                        peer.online ? 'Online' : 'Offline',
                        style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: peer.online ? AppColors.success : AppColors.error),
                      ),
                    ),
                  ],
                ),
          ...
          IconButton(
            ...
            tooltip: 'Rename',
            onPressed: () => _showRenamePeerDialog(context, ref, peer),
          ),
          IconButton(
            icon: Icon(Icons.delete_outline, size: 16, color: AppColors.error),
            tooltip: 'Remove',
            onPressed: () => _showDeletePeerDialog(context, ref, peer),
          ),
        ],
      ),
    );
        ...
        content: Column(
          ...
          children: [
            TextField(
              ...
              decoration: const InputDecoration(labelText: 'New hostname', helperText: 'Alphanumeric + dashes', border: OutlineInputBorder()),
              autofocus: true,
            ),
          ],
        ),
        actions: [
          ...
          FilledButton(
            onPressed: () async {
              ...
              if (newName.isEmpty || newName == peer.hostname) { Navigator.pop(ctx); return; }
              Navigator.pop(ctx);
              final client = ref.read(vpnStatusClientProvider);
              try {
                await client.renamePeer(peer.nodeId, newName);
                ref.invalidate(vpnPeersProvider);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Renamed to $newName'), backgroundColor: AppColors.success));
                }
              } catch (e) {
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Rename failed: $e'), backgroundColor: AppColors.error));
                }
              }
            },
  ...
  void _showDeletePeerDialog(BuildContext context, WidgetRef ref, VpnPeerInfo peer) {
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Remove ${peer.hostname}.eyenet-vpn.local from the VPN mesh?\n\nThis device will lose VPN access.'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          FilledButton(
            ...
            onPressed: () async {
              ...
              final client = ref.read(vpnStatusClientProvider);
              try {
                await client.deletePeer(peer.nodeId);
                ref.invalidate(vpnPeersProvider);
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('${peer.hostname} removed'), backgroundColor: AppColors.success));
                }
              } catch (e) {
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Remove failed: $e'), backgroundColor: AppColors.error));
                }
              }
            },
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              const Expanded(child: Text('Switch Tailscale to EyeNet', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.warning))),
            ],
          ),
          const SizedBox(height: 8),
          Text(
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 8),
          Expanded(child: Text('VPN Unavailable: $reason', style: const TextStyle(fontSize: 13, color: Colors.grey))),
        ],
      ),
    );
  }
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            TextField(controller: controller, decoration: const InputDecoration(labelText: 'Hostname', helperText: 'Alphanumeric + dashes, max 63 chars', border: OutlineInputBorder()), autofocus: true),
            const SizedBox(height: 8),
            Text('MagicDNS: ${controller.text}.eyenet-vpn.local', style: const TextStyle(fontSize: 12, color: AppColors.textSecondary)),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('Cancel')),
          FilledButton(
            onPressed: () async {
              ...
              if (newHostname.isEmpty || newHostname == currentHostname) { Navigator.pop(ctx); return; }
              Navigator.pop(ctx);
              try {
                final client = ref.read(vpnStatusClientProvider);
                ...
                if (context.mounted) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Hostname changed to $newHostname'), backgroundColor: AppColors.success));
                }
              } catch (e) {
                if (context.mounted) {
  ...
  Widget _buildVersionCell(ServiceInfo service) {
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      child: Text(service.version, style: const TextStyle(fontSize: 11, fontFamily: 'monospace', color: AppColors.textSecondary), overflow: TextOverflow.ellipsis, maxLines: 1),
    );
  }
}
        ...
        children: [
          Row(
            children: [
              ...
              const SizedBox(width: 8),
              const Expanded(child: Text('VPN Not Connected', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.warning))),
            ],
          ),
          const SizedBox(height: 8),
          ...
          if (_key != null && _key!.enrolled) ...[
            Container(
              width: double.infinity,
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(color: AppColors.success.withValues(alpha: 0.1), borderRadius: BorderRadius.circular(6), border: Border.all(color: AppColors.success)),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Row(children: [Icon(Icons.check_circle, color: AppColors.success, size: 18), SizedBox(width: 8), Expanded(child: Text('Enrolled!', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w700, color: AppColors.success)))]),
                  const SizedBox(height: 6),
                  if (_key!.tailscaleIp != null) Text('VPN IP: ${_key!.tailscaleIp}', style: const TextStyle(fontFamily: 'monospace', fontSize: 12, fontWeight: FontWeight.w600)),
                  if (_key!.matrixGroupId != null) Text('Matrix: ${_key!.matrixGroupId!.substring(0, 16)}...', style: const TextStyle(fontSize: 10, color: AppColors.textSecondary)),
                ],
              ),
            ),
            ...
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Row(children: [Icon(Icons.check_circle, color: AppColors.success, size: 16), SizedBox(width: 6), Text('Key ready — copy and run this command:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.success))]),
                  const SizedBox(height: 8),
                  SelectableText(_key!.tailscaleUpCommand, style: const TextStyle(fontFamily: 'monospace', fontSize: 11, color: AppColors.textPrimary)),
                  if (_key!.matrixGroupId != null) Padding(padding: const EdgeInsets.only(top: 4), child: Text('Matrix: ${_key!.matrixGroupId!.substring(0, 16)}...', style: const TextStyle(fontSize: 10, color: AppColors.textSecondary))),
                ],
              ),
            ),
          ] else if (_loading) ...[
          ...
          Row(
            children: [
              if (_key == null || !_key!.enrolled)
                ElevatedButton.icon(onPressed: _loading ? null : _getKey, icon: Icon(_key != null ? Icons.refresh : Icons.vpn_key, size: 16), label: Text(_key != null ? 'Retry' : 'Enroll Now'), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white)),
          ...
          if (_error != null) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(color: AppColors.error.withValues(alpha: 0.08), borderRadius: BorderRadius.circular(6), border: Border.all(color: AppColors.error.withValues(alpha: 0.3))),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
