  ...
  Future<bool> validateToken() async {
    ...
    try {
      ...
      if (JwtDecoder.isExpired(_jwtToken!)) {
        _logger.w('⚠️ JWT token is expired');
        return false;
      }
  ...
  void _scheduleTokenRefresh() {
    ...
    try {
      ...
      if (timeUntilRefresh.inSeconds > 0) {
        ...
        _logger.d('⏱️ Token refresh scheduled for: ${refreshTime.toIso8601String()}');
      } else {
        // Token expires soon, refresh immediately
        _logger.w('⚠️ Token expires soon, refreshing immediately');
        refreshToken();
      }
    } catch (e, stackTrace) {
      _logger.e('❌ Failed to schedule token refresh', error: e, stackTrace: stackTrace);
    }
  }
