...
/// the player's own embedded Tailscale node (Phase 4).
///
/// [Tailscale.instance.http.client] is a `package:http` client whose requests
/// travel over the tailnet — LAN-direct when co-located, DERPF-relayed otherwise.
/// Bridging it to dio lets the existing `SignageApiClient` and
/// `SignageDiscoveryService` (both dio-based) use the mesh without rewrites.
///
/// This is best-effort: the caller only installs it when the embedded node is up
/// ([TailscaleService.isUp]).
class TailscaleHttpClientAdapter implements HttpClientAdapter {
  ...
  Future<ResponseBody> fetch(
    ...
    // delivers it via requestStream; we fall back to options.data for the rare
    ...
    // 2. Send over the tailnet (the player's own node) and adapt the response.
