...
class AuthenticationService {
  ...
  Future<void> _tryAutoDiscoverServer() async {
    // Auto-discovery candidates - try localhost first since services bind to all interfaces
    // The test file shows localhost:8001 works for login, then we get dynamic URLs from platform services
    const List<String> candidates = [
      'http://localhost:8001',      // Primary: Works for login, then we get dynamic URLs
      'http://127.0.0.1:8001',      // Fallback localhost
      // Note: After successful login, we'll get actual service URLs from /api/v1/users/platform/services
    ];
