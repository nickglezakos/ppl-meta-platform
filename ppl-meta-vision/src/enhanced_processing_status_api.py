...
Flutter workflow @enhanced_status_router.get(
...
async def get_widget_processing_status(
    ...
    try:
        ...
        logger.info(
            ...
            media_uuid, user_uuid
        )s the existing processing status API
...
def validate_user_authentication(authorization: str = Header(None, alias="Authorization")):
...
def get_enhanced_processing_api() -> Workflow5ProcessingStatusAPI:
    """Get or create enhanced processing API instance."""
    from workflow5_cache_manager import Workflow5CacheManager
    from workflow5_data_access import Workflow5DataAccess
    from workflow5_processing_status_api import Workflow5ProcessingStatusAPI

    # Workflow5ProcessingStatusAPI creates its own internal dependencies
...
async def get_widget_processing_analytics(
    ...
    try:
        ...
        if completed_sessions:
            total_duration = sum(
                s.duration_seconds for s in completed_sessions 
