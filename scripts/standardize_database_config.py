...
class DatabaseConfig:
    ...
    def validate_connection_string(self, url: str) -> Tuple[bool, List[str]]:
        ...
        if parsed["port"] == "5432" and "localhost" in parsed["hostname"]:
            issues.append(
                "Using default PostgreSQL port 5432, should be 5433 for Docker external access"
            )
    ...
    def analyze_current_configs(self) -> Dict[str, Dict[str, any]]:
        ...
        for service, files in configs.items():
            print(f"\n📋 Service: {service}")
            service_results = {"files": [], "database_urls": [], "issues": []}

            for file_path in files:
                file_info = {"path": file_path, "database_urls": [], "issues": []}
        ...
        for service, config in standardized_configs.items():
            ...
            try:
                ...
                updated_content = re.sub(
                    r"DATABASE_URL=.*", f"DATABASE_URL={localhost_url}", content
                )

                # Also update individual DB components for consistency
                db_name = config["database_name"]
                updated_content = re.sub(
                    r"DB_HOST=.*", f"DB_HOST=localhost", updated_content
                )
                updated_content = re.sub(
                    r"DB_PORT=.*", f"DB_PORT=5433", updated_content
                )
                updated_content = re.sub(
                    r"DB_NAME=.*", f"DB_NAME={db_name}", updated_content
                )
                updated_content = re.sub(
                    r"DB_USER=.*", f"DB_USER=nickadmin", updated_content
                )
                updated_content = re.sub(
                    r"DB_PASSWORD=.*", f"DB_PASSWORD=Kodikos@23", updated_content
                )
