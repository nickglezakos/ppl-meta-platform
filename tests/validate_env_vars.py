...
SERVICE_DEFAULTS = {
    "ppl-meta-gateway": {"PORT": "8080", "MAIL_FROM_NAME": "PPL Meta Gateway"},
    "ppl-meta-node": {"PORT": "8001", "MAIL_FROM_NAME": "PPL Meta Node"},
    "ppl-meta-media": {"PORT": "8000", "MAIL_FROM_NAME": "PPL Meta Media"},
    "ppl-meta-orchestrator": {
        "PORT": "8002",
    ...
    for category, vars_in_category in categories.items():
        ...
        if applicable_vars:
            ...
            for var_name in applicable_vars:
                var_config = STANDARD_ENV_VARS[var_name]
                default_value = SERVICE_DEFAULTS.get(service, {}).get(
                    var_name, var_config["default"]
                )
...
def main():
    ...
    for service in services:
        ...
        if any(issues.values()):
            if issues["missing_required"]:
                print(
                    f"  ❌ Missing required variables: {', '.join(issues['missing_required'])}"
                )
            if issues["missing_optional"]:
                print(
                    f"  ⚠️  Missing optional variables: {', '.join(issues['missing_optional'])}"
                )
        else:
    ...
    # Summary
    total_issues = sum(
        len(issues["missing_required"]) + len(issues["missing_optional"])
    ...
    if total_issues > 0:
        ...
        print(
            "  4. Run this script with --generate-templates to auto-generate templates"
        )
        return 1
    else:
