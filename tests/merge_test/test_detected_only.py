#!/usr/bin/env python3
"""
MVR Merge Logic Integration Test — Single Video (DRY RUN)
...
All mvr_people for the video are fetched REGARDLESS of is_orphaned /
merged_into_mvr_uuid, bypassing all old false merges.

Output:
  - results_detected_only.txt                full text report
  - det_g<N>_m<M>_<uuid>_<gender>.jpg        cropped face per person per group

Video: d46124c8-8944-4784-be9f-3e6314e1f0b8  (2026-03-01, ~14:56)

PYTHON: autonomous/ppl-meta-mini/venv/bin/python3
Run:    python test_detected_only.py
"""
...
def main() -> None:
    sep = "=" * 68
    print(sep)
    print("MVR Merge Test - SINGLE VIDEO (DRY RUN, gender guard enabled)")
    print(f"Video      : {VIDEO_UUID}")
    print(f"Threshold  : {SIMILARITY_THRESHOLD}  Gender gate: {GENDER_CONF_GATE}")
    print(sep)

    token = get_auth_token()

    print(f"\nFetching ALL MVR people from DB for video (including orphaned)...")
    mvr_people  = fetch_all_mvr_people_raw([VIDEO_UUID])
    ...
    if not mvr_people:
        print("ERROR: No MVR people found for video — aborting.")
        sys.exit(1)
    ...
    lines = [
        "MVR Merge Test - SINGLE VIDEO (DRY RUN, gender guard enabled)",
    ...
    for g_idx, group in enumerate(merge_groups):
        ...
        for m_idx, mvr in enumerate(group):
            ...
            label     = f"det_g{g_idx + 1:02d}_m{m_idx + 1:02d}_{uuid_str[:8]}_{gender}"
    ...
    out_path = os.path.join(OUTPUT_DIR, "results_detected_only.txt")
