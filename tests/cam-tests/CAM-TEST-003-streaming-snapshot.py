...
This test validates the video streaming and snapshot capture capabilities including:
1. Authentication with Node service
2. Camera detection and connection
3. Video streaming start/stop operations
4. Snapshot capture during streaming
5. Stream data validation
...
class StreamingTestSuite:
    ...
    def test_video_streaming(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"    Testing video stream data retrieval...")
                try:
    ...
    def test_concurrent_operations(self) -> bool:
        """Step 5: Test concurrent streaming and snapshot operations."""
        print("\n🔄 Step 5: Concurrent Operations Testing")

        if not self.connected_cameras:
            self.log_test("Concurrent Operations", False, "No cameras connected")
            return False
        ...
        try:
            ...
            # Start streaming
            print(f"    Starting stream for concurrent test...")
            response = requests.post(
                f"{self.base_url}/api/v1/streaming/{device_id}/start",
                headers=headers,
            )
            ...
            if response.status_code == 200:
                ...
                # Capture snapshot while streaming
                print(f"    Capturing snapshot during streaming...")
                ...
                if snapshot_response.status_code == 200:
                    ...
                    if "data" in snapshot_data:
                        ...
                        self.log_test(
                            "Concurrent Snapshot",
                            True,
                            "Snapshot captured during streaming",
                        )
                    else:
                        self.log_test(
                            "Concurrent Snapshot",
                    ...
                    self.log_test(
                        "Concurrent Snapshot",
                        False,
                        f"HTTP {snapshot_response.status_code}",
                    )
                ...
                self.log_test(
                    ...
                    False,
                    "Could not start stream for concurrent test",
                )
                return False
    ...
    def cleanup_resources(self) -> bool:
        """Step 6: Cleanup resources and disconnect cameras."""
