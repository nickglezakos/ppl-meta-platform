...
| Role | Machine | Job |
|------|---------|-----|
| **Operator Mac** | your MacBook (Tailscale mesh) | Reach Windows; run `gh` / verify manifests; optional local Flutter web build |
| **Windows install host** | `kvalvis-pc` → Tailscale IP **`100.64.0.23`** | Docker Desktop pull + `install-platform.bat` (normal path: **no local image builds**) |
| **Image builder** | GitHub Actions (`platform-release.yml`) **preferred** | Build/push nine `linux/amd64` images to GHCR |
| **Optional amd64 builder** | same Windows PC (WSL2 / Git Bash) | Only if Actions is blocked; run bash build/push scripts |

Policy intent: customer Windows boxes **pull** pinned images. Building on Windows is a **lab fallback**, not the product path.
...
## 1. Decide how images get into GHCR

### Path A — Preferred: GitHub Actions (from Mac)
...
### Path B — Fallback: build on Windows (native amd64)
