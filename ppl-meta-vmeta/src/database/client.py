...
class VmetaDatabaseClient:
    """
    Database client for vmeta enhanced person detection service.
    ...
    def __init__(self, config: Dict[str, Any]):
        """Initialize database client with configuration dictionary."""
        host = config["host"]
        port = config["port"]
...
async def create_vmeta_database_client(config: Dict) -> VmetaDatabaseClient:
    """Create and initialize vmeta database client."""

    client = VmetaDatabaseClient(config)
    await client.initialize()

    logger.info("vmeta database client created and initialized")
    return client
...
async def example_database_usage():
    ...
    db_client = await create_vmeta_database_client(config)
