...
class WorkflowService:
    ...
    def __init__(self, database_client, embedding_service, config: dict = None):
    ...
    async def _execute_face_detection_workflow(
        ...
        result = await self.embedding_service.start_session_based_face_detection(
            session_uuid=session_uuid,
    ...
    async def _execute_person_routes_workflow(
        ...
        routes_created = await self.embedding_service._generate_person_routes(
            session_uuid
        )
...
def create_phase1_router(
    workflow_controller: WorkflowService,
) -> APIRouter:
