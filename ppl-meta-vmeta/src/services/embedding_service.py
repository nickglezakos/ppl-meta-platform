...
async def example_enhanced_face_detection():
    """Example of Phase 1 enhanced face detection usage."""

    # Initialize enhanced vision service
    vision_service = EnhancedVisionService(
        database_client=db_client, config=PHASE1_VISION_CONFIG
    )

    # Generate unique session UUID
    session_uuid = str(uuid.uuid4())

    # Start session-based face detection
    results = await vision_service.start_session_based_face_detection(
        session_uuid=session_uuid,
        media_id="media-123",
        source_identifier="camera-stream-lobby",
    ...
    print(f"- Session: {results['session_uuid']}")
    print(f"- Faces detected: {results['faces_detected']}")
    print(f"- Faces with embeddings: {results['faces_with_embeddings']}")
    print(f"- Faces with distance: {results['faces_with_distance']}")
    print(f"- Person routes created: {results['person_routes_created']}")
